...
def get_data(url, data=None):
    ...
    titles=soup.select('.events-container-item__link')
    scr=soup.select('.events-container__wrap script')
    f=file(r"C:\Users\XIECHEN\Desktop\sc\honglingji.txt","a+")
    for i in range(0,len(titles)-1):
    	#名称
    	name=titles[i].get('data-event-name')
    	#类型
    	cate=titles[i].get('data-category')
    	print cate.decode("unicode-escape") 
    	#地点
    	location=titles[i].get('data-venue-name')
    	#时间
    	p1=r"(?=endDate).+?(?=,)"
    	pattern1=re.compile(p1)
    	enddate=(pattern1.findall(str(scr[i]))[0]).split('"')[2]
    	# print enddate
    	#postcode
    	p2=r"(?=postalcode).+?(?=,)"
    	pattern2=re.compile(p2)
    	postcode=(pattern2.findall(str(scr[i]))[0]).split('"')[2]
    	#streetname
    	p3=r"(?=streetaddress).+?(?=,)"
    	pattern3=re.compile(p3)
    	streetaddress=(pattern3.findall(str(scr[i]))[0]).split('"')[2]
    	str1=name+">"+cate.decode("unicode-escape") +">"+location+">"+enddate+">"+postcode+">"+streetaddress+"\n"
    	print str1
    	f.write(str1)
